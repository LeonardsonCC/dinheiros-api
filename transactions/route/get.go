...
func GetSingleTransactionHandler(c *gin.Context) {
	...
	cats, err := categoriesRepo.GetTransactionCategories(transactionID)
	if err != nil {
		rest.Err(c, "failed to get categories from this transaction", err)
		return
	}

	ts := make([]transactions.TransactionJson, len(txs))
	for i, tx := range txs {
		ts[i] = transactions.MapDomainToJson(tx, cats)
	}
